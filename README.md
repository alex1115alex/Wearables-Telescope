...
I spent the rest of my time this week working on the software, which I'm calling TelescopeOS for now... very original. In order to do this more efficiently, I ditched the microdisplay for now and started using a composite to USB capture card which lets me view the ESP32's video output on my computer. If that sounds like an extra step for little reason, you should try developing a GUI on a .24inch screen that's connected with a delicate ribbon cable that can't be replaced by April 29th.

Progress on the software is far from over, however, and now that the capture card is setup I'll be working on it the next few days. As of now I have a home screen which shows the time and the number of unread notifications, and a function for cycling through those notifications. I've also started the groundwork for a basic json API over bluetooth, which right now accepts types "notif" (notification), "time" (for time synchronization w/ phone), "temperature" (will be used to display the weather), and "next" (used to cycle notifications & includes option to dismiss).
...
* * time (currently just uses a placeholder until Android App is ready)
